...
export const api = {
  ...
  getIdMonthlyCosts: async (params = {}) => {
    const result = await getIdMonthlyCostsReport(params);
    return {
      ...result,
      accounts: result.accounts.map(account => ({
        ...account,
        months: account.months.map(m => ({
          ...m,
...
export function getTxRowDetails(tx) {
  ...
  if (tx.type === 'expense' || (tx.type === 'revenue' && tx.revenue_source === 'other')) {
    const paidLabel = tx.paid_from ? PAID_FROM_LABELS[tx.paid_from] || tx.paid_from : '';
    const secondaryParts = [tx.category, paidLabel].filter(Boolean);
    return {
  ...
  const paidLabel = tx.paid_from ? PAID_FROM_LABELS[tx.paid_from] : '';
  const secondaryParts = [
    tx.description && tx.description !== primary ? tx.description : '',
    paidLabel,
  ].filter(Boolean);
