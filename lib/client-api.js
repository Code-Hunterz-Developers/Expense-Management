...
export const api = {
  ...
  createTransaction: (data) => createTransaction({ ...data, currency: currencyForType(data.type) }),
  updateTransaction: async (id, data) => {
    const existing = await getTransaction(id);
    if (!existing) throw new Error('Transaction not found');
    const txType = data.type || existing.type;
    return updateTransaction(id, { ...data, currency: currencyForType(txType) }, existing);
  },
...
export { currencyForType } from '@/lib/settings';
...
export function formatRevenueWithPkr(usdAmount, rate = 267, pkrOverride = null) {
...
export function formatRevenueSplit(usdAmount, rate = 267, pkrOverride = null) {
  const usd = Number(usdAmount) || 0;
  const pkr = pkrOverride !== null && pkrOverride !== undefined
    ? Number(pkrOverride)
    : usd * rate;
...
export function usdToPkr(usdAmount, rate = 267) {
