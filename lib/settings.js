...
export async function updateSettings(body) {
  ...
  if (usd_to_pkr !== undefined) {
    ...
    updates.investment_usd_to_pkr = rate;
  }
...
export async function getDashboardSummary({ year, month }) {
  ...
  const totalRevenueUsd = sumTypeAmount(revenueTxs, 'revenue');
  const total_investment = sumTypePkrCompanyPaid(transactions, 'investment', exchangeRate);
  const total_salary = sumTypePkr(transactions, 'salary', exchangeRate);
  const total_expense = sumTypePkrCompanyPaid(transactions, 'expense', exchangeRate);
  const totalOutflowPkr = total_investment + total_salary + total_expense;

  const calculatedPkr = revenueTxs.reduce((sum, tx) => sum + revenuePkrForTx(tx), 0);
  ...
  const effectiveRate = totalRevenueUsd > 0 ? displayPkr / totalRevenueUsd : 0;
  const revenueInAccUsd = effectiveRate > 0 ? revenueInAccPkr / effectiveRate : 0;

  return {
    exchange_rate: exchangeRate,
    revenue_withdrawal_rate: effectiveRate,
    revenue_usd: {
