...
export async function getDashboardSummary({ year, month }) {
  ...
  const totalRevenueUsd = sumType(transactions, 'revenue');
  const total_investment = sumType(transactions, 'investment');
  const total_salary = sumType(transactions, 'salary');
  const total_expense = sumType(transactions, 'expense');
  const totalOutflowPkr = total_investment + total_salary + total_expense;

  const calculatedPkr = totalRevenueUsd * exchangeRate;
  ...
  const revenueInAccUsd = exchangeRate > 0 ? revenueInAccPkr / exchangeRate : 0;
