...
export async function ensureSeedData() {
  ...
  if (!settingsSnap.exists()) {
    await setDoc(settingsRef, { usd_to_pkr: 267 });
  }
}
...
export async function getSettingsDoc() {
  ...
  return snap.exists() ? snap.data() : { usd_to_pkr: 267 };
}
...
export async function listAccounts() {
  ...
  return accounts.map(account => {
    ...
    const total_investment = sumByType(related, 'investment');
    const total_revenue = sumByType(related, 'revenue');
    const total_expense = sumByType(related, 'expense');
    return {
      ...account,
      total_investment,
      total_revenue,
...
export async function getMonthlyReport(year) {
  const transactions = filterByDate(await getAllTransactionsRaw(), { year: String(year) });
  const byMonth = {};
  ...
  for (const tx of transactions) {
    ...
    if (!month) continue;
    if (!byMonth[month]) {
      byMonth[month] = { month, investment: 0, revenue: 0, salary: 0, expense: 0 };
    }
    byMonth[month][tx.type] = (byMonth[month][tx.type] || 0) + (Number(tx.amount) || 0);
  }
...
export async function getByAccountReport({ year, month }) {
  const accounts = await getAllAccountsRaw();
  const transactions = filterByDate(await getAllTransactionsRaw(), { year, month });

  return accounts.map(account => {
    const related = transactions.filter(t => t.account_id === account.id);
    const investment = sumByType(related, 'investment');
    const revenue = sumByType(related, 'revenue');
    const expense = sumByType(related, 'expense');
    return {
      id: account.id,
      name: account.name,
      investment,
...
export async function getAccountMonthlyReport(accountId, year) {
  ...
  for (const tx of transactions) {
    ...
    if (tx.type === 'investment') byMonth[month].investment += Number(tx.amount) || 0;
    if (tx.type === 'revenue') byMonth[month].revenue += Number(tx.amount) || 0;
    if (tx.type === 'expense') byMonth[month].expense += Number(tx.amount) || 0;
  }
