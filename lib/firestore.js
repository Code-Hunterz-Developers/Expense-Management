...
export async function listAccounts() {
  ...
  const total_investment = sumTypePkrCompanyPaid(related, 'investment', exchangeRate);
    const total_revenue = sumByType(related, 'revenue');
    const total_revenue_pkr = related
      .filter(t => t.type === 'revenue')
      .reduce((sum, t) => sum + revenuePkrForTx(t), 0);
    const total_expense = sumTypePkrCompanyPaid(related, 'expense', exchangeRate);
    return {
      ...account,
      total_investment,
      total_revenue,
...
export async function createTransaction(data) {
  ...
  const payload = {
    ...
    exchange_rate: ['investment', 'expense'].includes(data.type) && data.currency === 'USD'
      ? (data.exchange_rate != null ? parseFloat(data.exchange_rate) : null)
      : null,
    exchange_pkr: ['investment', 'expense'].includes(data.type) && data.currency === 'USD'
      ? (data.exchange_pkr != null && data.exchange_pkr !== '' ? parseFloat(data.exchange_pkr) : null)
      : null,
...
export async function updateTransaction(id, data, existing) {
  ...
  const payload = {
    ...
    exchange_rate: ['investment', 'expense'].includes(txType) && (data.currency ?? existing.currency) === 'USD'
      ? (data.exchange_rate !== undefined ? data.exchange_rate : (existing.exchange_rate ?? null))
      : null,
    exchange_pkr: ['investment', 'expense'].includes(txType) && (data.currency ?? existing.currency) === 'USD'
      ? (data.exchange_pkr !== undefined ? data.exchange_pkr : (existing.exchange_pkr ?? null))
      : null,
...
export async function getMonthlyReport(year) {
  ...
  for (const tx of transactions) {
    ...
    } else if (tx.type === 'investment' && isCompanyPaidCost(tx)) {
    ...
    } else if (tx.type === 'expense' && isCompanyPaidCost(tx)) {
...
export async function getByAccountReport({ year, month }) {
  ...
  return accounts.map(account => {
    const related = transactions.filter(t => t.account_id === account.id);
    const investment = sumTypePkrCompanyPaid(related, 'investment', exchangeRate);
    const revenue = sumByType(related, 'revenue');
    const revenue_pkr = related
      .filter(t => t.type === 'revenue')
      .reduce((sum, t) => sum + revenuePkrForTx(t), 0);
    const expense = sumTypePkrCompanyPaid(related, 'expense', exchangeRate);
    return {
      id: account.id,
      name: account.name,
      investment,
...
export async function getAccountMonthlyReport(accountId, year) {
  ...
  for (const tx of transactions) {
    ...
    const amount = Number(tx.amount) || 0;
    if (tx.type === 'investment' && isCompanyPaidCost(tx)) {
      byMonth[month].investment += txAmountPkr(tx, exchangeRate);
    }
    if (tx.type === 'revenue') byMonth[month].revenue += amount;
    if (tx.type === 'expense' && isCompanyPaidCost(tx)) {
      byMonth[month].expense += txAmountPkr(tx, exchangeRate);
    }
  }
...
export async function importFromSqlite({ accounts, transactions, settings }) {
  ...
  for (const tx of transactions) {
    ...
    batch.set(ref, {
      ...
      exchange_rate: ['investment', 'expense'].includes(tx.type) && (tx.currency || 'PKR') === 'USD'
        ? (tx.exchange_rate ?? null)
        : null,
      exchange_pkr: ['investment', 'expense'].includes(tx.type) && (tx.currency || 'PKR') === 'USD'
        ? (tx.exchange_pkr ?? null)
        : null,
