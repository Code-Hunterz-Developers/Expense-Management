...
/** Investment/expense paid from company pocket — excludes ID Balance costs */
export function isCompanyPaidCost(tx) {
  if (tx?.type !== 'investment' && tx?.type !== 'expense') return true;
  return normalizePaidFrom(tx.paid_from) === PAID_FROM.company;
}

export function sumTypePkrCompanyPaid(transactions, type, exchangeRate) {
  return transactions
    .filter(t => t.type === type && isCompanyPaidCost(t))
    .reduce((sum, t) => sum + txAmountPkr(t, exchangeRate), 0);
}
